...
`/scan/folder` takes a caller-supplied path. On a public host that is a path-traversal and information-disclosure surface. Two things must both be true before this goes live:

1. `ARGUS_CURATOR_SCAN_ROOT` confines scanning to `/srv/argus/samples`, mounted read-only into the container.
2. **Curator enforces that server-side**, treating the path as untrusted input. `NEXT_PUBLIC_CURATOR_UI_MODE=demo` is a frontend flag, not an authorization boundary — anyone can curl the API directly.

Item 2 is not done. It lives in argus-curator, not here.
