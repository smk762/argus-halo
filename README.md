...
**What is actually reachable.** One origin, one namespace per service: Caddy routes `/api/lens/*` to lens and `/api/curator/*` to curator, stripping the prefix so each backend still sees its own paths (`/api/curator/scan/folder` arrives as `/scan/folder`). Everything else falls through to the frontend. Reachability does **not** depend on how studio was built: `demo` mode only means the bundled frontend doesn't call these paths, not that they're closed. Anyone with curl reaches them either way.
...
- `ARGUS_CURATOR_SCAN_ROOT` points at `/srv/argus/samples`, mounted read-only into the container. `ARGUS_CURATOR_EXPORT_ROOT` is left empty, so `/export` refuses server-side — whether it should be reachable at all is [#19](https://github.com/smk762/argus-halo/issues/19).
